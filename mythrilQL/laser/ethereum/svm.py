...
from mythril.laser.ethereum.state.execution_state import ExecutionState
...
from mythril.laser.ethereum.q_learning import *
...
class LaserEVM:
    ...
    def sym_exec(
        ...
        """
        print('开始符号执行,gloabalstate为:',vars(GlobalState))
        # print('执行一次sym_exec')
    ...
    def exec(self, create=False, track_gas=False) -> Optional[List[GlobalState]]:
        ...
        sensitive_instruction = ['CALL', 'STATICCALL', 'CALLCODE', 'DELEGATECALL', 'SELFDESTRUCT', 'SSTORE']
        candidatel_list  = [] #候选列表
        sensitive_list =  [] #遇到的敏感指令
        q_table = build_q_table(1, ['true', 'false'])  # 建立Q表

        candidatel_list.append(self.strategy.__next__())#将初始状态放到候选状态列表
        while candidatel_list != []:
            #选取分数最大的global_state
            candidatel_list,global_state = get_max_score_state(candidatel_list)
            print('global_state',type(global_state))
        # for global_state in self.strategy:
            S = 0  # 初始化状态
            is_terminated = False  # 将到达终点设为false以结束循环
            while not is_terminated:  # 若没有探索完该条路径则一直探索
                ...
                try:
                    print('执行一次execute_state')
                    new_states, op_code = self.execute_state(global_state)
                    S = global_state.mstate.pc
                    print(S)
                    #如果执行的指令是JUMPI,从new_states选一个Q值最大的状态继续执行，
                    #将另一个状态加入候选状态列表
                    if op_code == 'JUMPI' and len(new_states)==2:
                       A,q_table  = choose_action(S,q_table)
                       #计算R
                       R = len(sensitive_list)
                       sensitive_list = []
                       S_ = 0
                       ...
                       #另一个状态加入候选状态列表
                       if A == 'true':
                           candidatel_list.append(new_states[1])
                           new_states = [new_states[0]]
                       else:
                           candidatel_list.append(new_states[0])
                           new_states = [new_states[1]]
                        #更新Q表
                       q_table.loc[S, A] += ALPHA * (q_target - q_predict)  # 更新Q[S,A]值
                    #若执行的指令是终止指令，is_terminated=ture
                    if op_code in ["STOP","REVERT","RETURN"]:
                        is_terminated = True
                        break
                    #不是上述两者，将敏感指令存到sensitive_list，更新global_state
                    oopp = global_state.environment.code.instruction_list[global_state.mstate.pc]["opcode"]
                    if oopp in sensitive_instruction:
                        sensitive_list.append(oopp)
                ...
                if new_states:
                    ExecutionState.execution_depth += 1
                    self.work_list += new_states
                    for i in new_states:
                        print('svm.py 向work_list添加了:', i.environment.code.instruction_list[i.mstate.pc]["opcode"])
                elif track_gas:
                    final_states.append(global_state)
                self.total_states += len(new_states)

                try:
                    global_state = self.strategy.__next__()
                except:
                    break
    ...
    def execute_state(
        ...
        #print('666',global_state.environment.code.instruction_list)
        try:
            op_code = instructions[global_state.mstate.pc]["opcode"]
            print('当前执行指令:', global_state.mstate.pc,op_code)
            #TODO fwl获取父节点的score，判断是否属于敏感指令，更新当前score，计算Priority
            #定义敏感指令集合,定义权重α和β，阈值Q
            beita = 1#路径权重β，目前是1
            Q = 0.00#优先级阈值，目前是0.01

            #计算priority,priority= score / (depth *β)
            priority = global_state.score / (global_state.execution_depth * beita)
            print('当前score为:', global_state.score,end=" | ")
            print('当前depth为：', global_state.execution_depth,end=" | ")
            ...
            #todo fwl状态感知
            #SLOAD:栈顶出栈,并以该值作为storage的索引，加载该索引对应的值放到栈顶
            # if (op_code == 'JUMPI'):
            #     data = global_state.mstate.stack[-1]
            #     print('将要执行JUMPI，当前栈顶为', type(data))
        ...
        self._execute_post_hook(op_code, new_global_states)
        #print('返回了状态', new_global_states, op_code)
        #fwl深度增加,分数继承

        #定义敏感指令
        sensitive_instruction = ['CALL', 'STATICCALL', 'CALLCODE', 'DELEGATECALL', 'SELFDESTRUCT', 'SSTORE']
        #每次遇到敏感指令，score+α，当前α=10
        aerfa = 10

        #遍历符号执行产生的符号状态
        for i in range(len(new_global_states)):
            new_global_states[i].execution_depth = global_state.execution_depth+1
            new_op = new_global_states[i].environment.code.instruction_list[new_global_states[i].mstate.pc]["opcode"]
            #如果执行的是敏感指令，当前路径score+α
            if new_op in sensitive_instruction:
                new_global_states[i].score = global_state.score + aerfa
                print(new_op, '为敏感指令，score+10,当前score为:', new_global_states[i].score)
            else:
                new_global_states[i].score = global_state.score

        #todo fwl判断是否执行了sload指令
        #SLOAD:栈顶出栈,并以该值作为storage的索引，加载该索引对应的值放到栈顶
        # if(op_code=='SLOAD'):
        #     data = new_global_states[0].mstate.stack[-1]
        #     print('从区块链读取了数据，该数据为：',data)
