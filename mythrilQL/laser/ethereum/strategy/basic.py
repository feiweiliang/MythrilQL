...
class BreadthFirstSearchStrategy(BasicSearchStrategy):
    ...
    def get_strategic_global_state(self) -> GlobalState:
        ...
        for i in self.work_list:
            print('work_list有:',i.environment.code.instruction_list[i.mstate.pc]["opcode"])
        aaa = self.work_list.pop(0)
        #print('出队：',aaa.environment.code)
        return aaa#pop(0)弹出第一个元素，即队列出队
...
class QLearingGuidedSearchStrategy(BasicSearchStrategy):
    """使用Q学习指导符号执行搜索
    """
